...
async function loadServers() {
  ...
  if (currentServerId) {
    ...
    if (currentServer) {
      return await updateCurrentServerStatus();
    }
  }
  return false;
}
  ...
  for (const server of servers) {
    ...
    try {
      ...
      if (server.id === currentServerId) { 
        status = serverStatusCache.get(currentServerId);
...
async function updateServerDropdown() {
  ...
  */
  ...
  servers.forEach(server => {
    ...
    dropdownItem.innerHTML = `
      <span class="server-name">${displayName}</span>
      <span class="dropdown-status unknown">${getMessage('unknownStatus')}</span>
    `;
...
async function updateCurrentServerStatus() {
...
async function checkServerConnection(serverId = currentServerId) {
  ...
  if (serverId === currentServerId) {
    setServerStatus(serverId, 'checking');
  }

  if (serverId === currentServerId && connectionCheckPromise) {
...
function updateConnectionUI(status) {
  ...
  const noSettings = document.getElementById('noSettings');
  const downloadsSection = document.getElementById('downloadsSection');

  if (!connectionStatus || !noSettings) return;
  ...
  if (status.success) {
    ...
    if (status.authenticated) {
      let message = '✅ ' + getMessage('connectedAndAuthenticated');
      if (status.version) {
      ...
      connectionStatus.textContent = '⚠️ ' + getMessage('connectedNeedAuth');
      ...
      connectionStatus.textContent = '✅ ' + getMessage('connected');
      connectionStatus.className = 'status success';
    }
  } else {
    connectionStatus.textContent = '❌ ' + getMessage('connectionFailed', [status.error || getMessage('unknownError')]);
...
function deleteDownload(hash, name) {
  const confirmTitle = getMessage('confirmDeleteTitle') || 'Confirm Delete';
