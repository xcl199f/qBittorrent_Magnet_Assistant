...
class DownloadManager {
  ...
  updateActiveDownloads(torrents) {
    ...
    torrents.forEach(torrent => {
      const isDownloading = ['downloading', 'forcedDL'].includes(torrent.state);
    ...
    if (activeCount > 0) {
      ...
      this.activeDownloads.forEach(download => {
        ...
        if (download.progress < 1 && download.downloadSpeed === 0 && 
            (download.state === 'stalledDL' && Date.now() - new Date(download.addedTime).getTime() > 30000
             || download.state === 'downloading')) {
          hasStalledDownloads = true;
          console.log(download,'stall')
          badgeColor = '#c15448ff';
        }
      });
